...
We take the security of this OCPP 1.6 library seriously. If you discover a
...
Instead, please report security vulnerabilities using one of the following
methods:

1. **GitHub Security Advisories (Preferred)**
   - Navigate to the [Security Advisories](https://github.com/aasanchez/ocpp16messages/security/advisories)
     page
...
- Type of vulnerability (e.g., input validation, authentication bypass,
  injection)
...
This library provides OCPP 1.6 message type definitions and validation for Go
applications. Security vulnerabilities in scope include:

#### In Scope

- **Input Validation Failures:** Bypass of length constraints, character
  encoding issues
- **Injection Vulnerabilities:** SQL injection, command injection, or similar
  attacks via message fields
...
- **Protocol Implementation Flaws:** Violations of OCPP 1.6 spec that create
  security risks
...
- Application-level authorization logic (business logic in consuming
  applications)
...
5. **Sanitize logs:** Be careful when logging message contents that may contain
   sensitive IDTokens
6. **Keep dependencies updated:** Regularly update to the latest version for
   security patches
7. **Thread safety:** All types use value receivers and are designed for
   concurrent use - don't modify after creation
...
We appreciate the security research community's efforts to responsibly disclose
vulnerabilities. Contributors who report valid security issues will be
acknowledged in:
