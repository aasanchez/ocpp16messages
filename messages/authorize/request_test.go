...
func TestAuthorizeRequest_emptyIdTag(t *testing.T) {
	...
	input := mat.RequestPayload{IdTag: ""}
	if err := input.Validate(); err == nil {
	...
	if !strings.Contains(err.Error(), expected) {
		t.Errorf(st.ErrContainsFmt, expected, err.Error())
	}
}
...
func TestAuthorizeRequest_tooLongIdTag(t *testing.T) {
	...
	input := mat.RequestPayload{IdTag: strings.Repeat("A", 21)}
	_, err := Request(input)
...
func TestAuthorizeRequest_nonASCIIIdTag(t *testing.T) {
	...
	input := mat.RequestPayload{IdTag: "مرحباOCPP"}
	_, err := Request(input)
