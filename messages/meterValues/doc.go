...
//
// The Charge Point SHALL send MeterValues.req to offload meter data. Each request
// SHALL include the following for each sample:
//
...
//     is used.
//  3. meterValue: One or more MeterValue elements, each representing measurements
//     taken at a specific timestamp. Each MeterValue contains:
//     - timestamp: The time the data was captured.
//     - sampledValue: One or more individual measurement values. Optional fields:
...
//     - phase: Electrical phase(s) the measurement applies to. Values
//     SHALL be reported from the meter/grid perspective. Not applicable
//     to all Measurands.
...
//
// Notes:
//   - Two special Measurands, Current.Offered and Power.Offered, indicate the
//     maximum current/power offered to the EV for smart charging purposes.
//   - For connector phase rotation, the Central System MAY query the
//     ConnectorPhaseRotation configuration key via GetConfiguration. Values per
//     connector include: NotApplicable, Unknown, RST, RTS, SRT, STR, TRS, TSR.
//   - Default interpretation of a sampledValue without optional fields is a
//     register reading of active import energy in Wh (Watt-hour).
//
...
// to retry the message according to transaction-related error handling rules.
package meterValues
